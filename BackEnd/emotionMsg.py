...
db = client["pomodoro_app"]  # What is the database part the connects this 
emotion_collection = db["emotions"]

def fetch_emotion_data(session_id):
    emotions = list(emotion_collection.find({"sessionId": session_id}))
    return [
        {
            "emotion": e["emotion"],
        } for e in emotions
...
def get_overall_emotion(emotion_data):
    emotion_list = [entry["emotion"] for entry in emotion_data]
    if not emotion_list:
...
def send_emotion_notification(user_id, emotion, advice):
    ...
    print(f"[Saved] Notification logged for User: {user_id}")

def handle_emotion_session(session_id, user_id):
    emotion_data = fetch_emotion_data(session_id)
    overall_emotion = get_overall_emotion(emotion_data)
    advice = get_emotion_advice(overall_emotion)
    send_emotion_notification(user_id, overall_emotion, advice)
